...
//!
//! Derives the Captval trait requirements for a struct such as in the example
//! below of a contact form providing the captval response token, the
//! client ip address of the client and the captval service sitekey.
//!
//! ```rust
//! use captval::Captval;
//!
...
/// ```
#[proc_macro_error]
#[proc_macro_derive(Captval, attributes(captcha, remoteip, sitekey))]
pub fn captval_derive(input: TokenStream) -> TokenStream {
...
fn impl_captval(ast: &DeriveInput) -> TokenStream {
    let name = &ast.ident;
    ...
    let gen = quote! {
        impl #impl_generics Captval for #name #ty_generics #where_clause {
            fn valid_response(&self, secret: &str, uri: Option<String>) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<captval::Response, captval::Error>> >>  {
                let mut client = captval::Client::new();
                if let Some(u) = uri {
                    match client.set_url(&u) {
                        Ok(c) => client = c,
    ...
    match attributes.get(name) {
        Some(i) => {
            ...
            quote! {
                #[allow(unused_mut)]
                let mut captcha;
                match captval::Captcha::new(&self.#i) {
                    Ok(c) => captcha = c,
                    Err(e) => {
                        return Box::pin(async{Err(e)});
                    }
        ...
        struct MyStruct {
            ...
            proc_macro_error2::abort! {id,
                "Field containing captval not identified";
...
/// that identify the fields relevant to captval processing
...
mod tests {
    ...
    fn test_get_required_attribute_with_valid_field() {
        let mut attributes = HashMap::new();
        let field_ident = Ident::new("captval_field", Span::call_site());
    ...
    fn test_get_required_attribute_multiple_fields() {
        let mut attributes = HashMap::new();
        let captcha_field = Ident::new("captval_field", Span::call_site());
